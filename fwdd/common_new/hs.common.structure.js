...
function FrameWork() {
    ...
    /**
     *  描述：设置指定对象为获焦对象.
     *  holdObject参数对象:  1.holdOldGroupObj(是否调用区域中的保留焦点)
     *                      2.holdOldFoc(失焦是否执行)
    ...
    this.add = function (fObject) {
        ...
        this.touch_even(document.getElementById(oid), function (type, event, distance) {
            fwThis.isTouchFlag = true;
...
function AjaxObject() {
    ...
    var loadStuts = false;
    var that = this;
    this.urlsPrefix = [geturl1, geturl2, geturl3, geturl4];
    /**
     * 描述: 同步/异步,默认同步
     * @type {boolean}
     */
    ...
    /**描述: url前缀添加到数组中, 返回数组中的位置.
     *
     * @param prefix
     * @returns {number} ajax _type值
     */
    this.addUrlPrefix = function (prefix) {
        this.urlsPrefix.push(prefix);
        return this.urlsPrefix.length;
    };
    var init = function (_url, _type, _timeout) {
        ...
        if (typeof _type == 'number' && _type > 0) {
            ajaxUrl = window.location.hostname === 'localhost' ? that.urlsPrefix[_type - 1].replace('UTLService', 'UTLService2') + ajaxUrl : that.urlsPrefix[_type - 1] + ajaxUrl;
        }
    ...
    var formatParams = function (_method, _data) {
    ...
    var ajax_method = function (_method, _url, _data, _fun, _type, _err, _timeout) {
        ...
        if (ajaxUrl) {
            destroy();
            _data = formatParams(_method, _data);
     ...
     * @param _fun  成功的回调函数
     * @param _err  失败的回调函数
     * @param _timeout  超时时间,默认3000
     * @param _callbackName 回调函数的名字,默认callback.
     */
    this.jsonp = function (_url, _data, _fun, _err, _timeout, _callbackName) {
        var exec = false;
        _timeout = _timeout || 3000;
        // 设置回调函数名字
        var callbackName = _callbackName || 'callback';
        if (_data != null && typeof _data == 'object') {
            _data['callback'] = callbackName;
            _data = formatParams(_data);
        } else {
        ...
        //发送请求
        script.src = _url + '?' + _data;

        script.onload = script.onreadystatechange = function (evt) {
            // console.log(evt.type === "error" ? 404 : 200, evt.type );
            if (!exec) window[callbackName]();
            // 加载完后清除.
...
function MediaObject(fwThis) {
    ...
    this.goJSPlayPage = function (playUrl, id, title, module, account, domain) {
        domain = domain || '21.254.5.190';
        account = account || '';
        fwThis.go('http://' + domain + '/hsvod/vodplay2timer.htm?vod=' + encodeURIComponent(playUrl) + '&id=' + id +
    ...
    this.goPlayPage = function (playUrl, isSeek, ret, domain) {
        ...
        var url = 'http://' + domain + '/hsvod/vodplay.htm?vod=' + encodeURIComponent(playUrl);
        if (isSeek) {
            url = fwThis.locationUrl.setQueryString(url, 'seek', this.getElapsed());
        }
        url = fwThis.locationUrl.setQueryString(url, 'ret', ret);
        fwThis.go(url);
    };
    /**
     * 描述: 监控视频页面
    ...
    this.goMonitorPage = function (playUrl, domain) {
        domain = domain || '21.254.5.190';
        fwThis.go('http://' + domain + '/hsvod/vodplay2monitor.htm?vod=' + encodeURIComponent(playUrl));
    };
    /**
     *  描述：设置媒体并播放.
    ...
    this.setMedia = function (left_Media, top_Media, width_Media, height_Media, _loadUrl, _onPlay, _closeFun, _errFun) {
        try {
            ...
            /*播放结束*/
            fwThis.addEvenToList([5210], function () {
                _this.closeMedia();
            ...
            /*连接失败、搜索失败、播放失败*/
            fwThis.addEvenToList([5203, 5204, 5206], function (key_code) {
                _this.closeMedia();
    ...
    this.getSiteInfo = function (siteID, siteIndex, siteItem) {
        ...
        var response = fwThis.crossAjaxHandler({
            url: base_url,
            data: encodeURIComponent(data.replace(/</ig, '&lt;').replace(/>/ig, '&gt;')),
            method: 'post',
            type: 'soap'
        });
        if (response.indexOf('未查询到任何内容信息') != -1) {
            ...
            var xmlDoc = fwThis.xml.loadXML(response); //loadXML方法载入xml字符串
    ...
    this.getSite2Url = function (siteID, contentID, type) {
        ...
        var data = '<?xml version="1.0" encoding="UTF-8" ?>' +
            ...
            '</message>';
        var str = '';
        fwThis.ajax.post('', 'url=' + base_url + '&data=' + encodeURIComponent(data.replace(/</ig, '&lt;').replace(/>/ig, '&gt;')) + '&method=soap', function (response) {
            // console.log(response);
            if (types == 1) {
                var xmlDoc = fwThis.xml.loadXML(response);
    ...
    this.getConidToUrl = function (contentID, type) {
        ...
        var str = '';
        var res = fwThis.crossAjaxHandler({
            url: url,
        ...
        if (types == 1) {
            var xmlDoc = fwThis.xml.loadXML(res);
            var elements = xmlDoc.getElementsByTagName('play-urls');
            for (var i = 0; i < elements.length; i++) {
                var urls = elements[i].getElementsByTagName('play-url');
                for (var j = 0; j < urls.length; j++) {
...
function LocationUrlObject() {
    ...
    this.formatImgUrl = function (url, prefixSrc) {
        var regex = /src="([^"]*)"/gi;
        prefixSrc = prefixSrc || 'http://21.254.5.190';
        if (url != null && url != '') {
...
function MemoryObject(fwThis) {
    ...
    this.initMemoryO = function (backname, reslove, reject) {
        ...
        this.upBackName = fwThis.locationUrl.getQueryString('upBackName');
        if (this.upBackName) {
            this.upBackUrl = fwThis.cookie.get(this.upBackName);
        }
        var hasback = this.loadBefor();
        if (hasback && fwThis.isFunction(reslove)) reslove(this.backObj);
        if (!hasback && fwThis.isFunction(reject)) reject();
    ...
    this.getOutside = function (projectName, fileName) {
        ...
        var baseUrl = fwThis.locationUrl.getBaseUrl(projectName);
        fwThis.global.set('outside_returnUrl', self);
        fwThis.cookie.set('outside_returnUrl', self);
        return baseUrl + name;
    };
    /**
     * 描述: 返回事件默认处理.
    ...
    this.goBack = function () {
        // hs.logi('goBack->return:' + fwThis.locationUrl.getQueryString('returnUrl'));
        fwThis.cookie.delOne('hs_back_' + this.backName);
        if (fwThis.locationUrl.getQueryString('indexUrl')) {
            //省网门户二次编码,需要二次解码.
            window.location.href = decodeURIComponent(fwThis.locationUrl.getQueryString('indexUrl'));
        } else if (fwThis.locationUrl.getQueryString('returnUrl')) {
            window.location.href = fwThis.locationUrl.getQueryString('returnUrl');
        } else if (this.upBackUrl) {
            fwThis.cookie.delOne(this.upBackName);
    ...
    this.loadBefor = function () {
        var hs_back = fwThis.cookie.get('hs_back_' + this.backName);
        if (hs_back) {
            this.backObj = fwThis.json.parse(hs_back);
            this.loaded = true
        }
        return this.loaded;
    };
    ...
    this.setBack = function (bObj, isHold) {
        ...
        fwThis.cookie.set('hs_back_' + this.backName, fwThis.json.stringify(bObj));
        if (isHold) {
            fwThis.cookie.set(this.localhostName, location.href);
        }
    };
}
...
function ListenLive(fwThis) {
    ...
    this.setMw = function () {
        ...
        var sTimeR = fwThis.formatToTime(_this.nowTime.split(' ')[1]);
        sTimeR = parseInt(sTimeR) - _this.delayTime;
        var sTime = _this.formateTime(
            sTimeL + ' ' + fwThis.formatTime(sTimeR)
        );
        var eTime = _this.formateTime(_this.endTime);

        var url = fwThis.locationUrl.setQueryString(_this.playUrl, "start-time", sTime);
        url = fwThis.locationUrl.setQueryString(url, "stop-time", eTime)
        console.log(url)
        //  播放
        this.playUrl = url;
    ...
    this.setOldMw = function () {
        ...
        var sTimeR = parseInt(
            fwThis.formatToTime(_this.startTime.split(' ')[1])
        );
        var sTime = _this.formateTime(
            sTimeL + ' ' + fwThis.formatTime(sTimeR)
        );
        ...
        var eTimeR = parseInt(
            fwThis.formatToTime(_this.endTime.split(' ')[1])
        );
        var eTime = _this.formateTime(
            eTimeL + ' ' + fwThis.formatTime(eTimeR)
        );
        var url = fwThis.locationUrl.setQueryString(_this.playUrl, "start-time", sTime)
        url = fwThis.locationUrl.setQueryString(url, "stop-time", eTime)
...
function FrameWorkOtherExpand(fwThis) {
    ...
    this.createElem = function (type, prame, parentNode) {
        ...
        if (_this.isUndefined(parentNode)) {
    ...
    this.toggleDisplay = function (_e, _d) {
        if (_this.isString(_d)) {
            var val = (_d === 'show' ? 'block' : 'none');
            _this.css(_e, {display: val});
            return true;
        }
    };
    /**
     ...
     */
    this.deepCopy = function (target, obj) {
        if (typeof obj === "undefiend") {
            obj = target;
            target = {}
        }
        for (var prop in obj) {
                    ...
                    if (data instanceof Array) {
                        ...
                        for (var i = 0; i < data.length; i++) {
                            if (typeof data[i] !== "object") {
                                target[prop].push(data[i])
                            } else {
                                target[prop][i] = _deepCopy({},
                                    data[i])
                            }
                        }
                    } else {
                        target[prop] = _deepCopy({},
                            data)
                    }
                }
            }
     ...
     */
    this.addRecord = function (id, code, isTW, title, domain) {
        //增加记录功能
        var type = window.location.hostname === 'localhost' ? 'UTLService2' : 'UTLService';
        var url = 'http://' + (domain || '21.254.5.190') + '/' + type + '/UIL/Trace/Handler.ashx';
        fwThis.ajax.get(url + '?stbid=' + fwThis.mw.getSTBID() + '&is_tw=' + isTW + '&cid=' + id + '&ctitle=' + title + '&mcode=' + code, function (data) {
            console.log(data);
        }, 0, function (msg) {
        }, 3000);
    };
    ...
    this.getVedioByDlw = function (parame) {
        ...
        fwThis.ajax.get(url, function (res) {
            str = res;
        }, 0, function (msg) {
            console.log(msg);
        }, 3000);
    ...
    this.crossAjaxHandler = function (parame) {
        ...
        fwThis.ajax.post('http://' + parame.domain + '/' + (window.location.hostname == 'localhost' ? 'UTLService2' : 'UTLService') + '/UIL/Cross/Handler2.ashx',
    ...
    this.showMsgInfo = function (_imgurl, _msg, _callback, _time) {
        ...
        _this.toggleDisplay($('hs_msg'), 'show');
        if (_time) {
            setTimeout(function () {
            ...
            fwThis.addEvenToList(hs.defaultDoCode, function () {
                _this.toggleDisplay($('hs_msg'), 'hidde');
                if (typeof _callback == 'function') {
                    _callback();
                }
            });
            fwThis.addEvenToList(hs.defaultBackCode, function () {
                _this.toggleDisplay($('hs_msg'), 'hidde');
                if (typeof _callback == 'function') {
                    _callback();
                }
    ...
    this.showKey = function (_left, _top, _keyArea, _objId) {
        ...
        _left = _left || 909;
        _top = _top || 166;
        if (_this.isDomExist('hs_keyboard')) {
            ...
            _this.forEach(key, function (item, i) {
                // i 从1开始
                i += 1;
                new fObject(_keyArea, i, "user_key_" + i,
                    ...
                    function () {
                        if (_this.arrayIsContains(leftArr, fwThis.focObj.objindex) > -1) {
                            fwThis.initFocus(fwThis.areaLastFocus[fwThis.prevArea].objid);
                        } else if (fwThis.focObj.objindex > 1) {
                            fwThis.initFocus("user_key_" + (fwThis.focObj.objindex - 1));
                        }
                    },
                    function () {
                        if (_this.arrayIsContains(rightArr, fwThis.focObj.objindex) > -1) {
                            fwThis.initFocus(fwThis.areaLastFocus[fwThis.prevArea].objid);
                        } else if (fwThis.focObj.objindex < key.length) {
                            fwThis.initFocus("user_key_" + (fwThis.focObj.objindex + 1));
                        }
                    });
    ...
    this.show9Key = function (_left, _top, _width, _height, _keyArea, _target, _fontSize, _head) {
        ...
        if (_this.isDomExist('hs_keyboard')) {
            ...
            for (var k = 0; k < 12; k++) {
                new fObject(_keyArea, k, 'hs_keyboard_' + k,
                    function (obj) {
                        if (obj.objindex > 2) {
                            fwThis.initFocus('hs_keyboard_' + (obj.objindex - 3));
                        } else if (_head.top) {
                            if (typeof _head.top == 'function') {
                                _head.top();
                            } else {
                                fwThis.initFocus(_head.top)
                            }
                        }
                    },
                    function (obj) {
                    ...
                    function (obj) {
                        if (_head.left && _this.arrayIsContains([0, 3, 6, 9], obj.objindex) != -1) {
                            if (typeof _head.left == 'function') {
                                ...
                                fwThis.initFocus(_head.left)
                            }
                        } else if (obj.objindex > 0) {
                            fwThis.initFocus('hs_keyboard_' + (obj.objindex - 1));
                        }
                    },
                    function (obj) {
            ...
            fwThis.addFocus([_keyArea, _keyArea + 1], function (group, id, index) {
            ...
            fwThis.addBlur([_keyArea, _keyArea + 1], function (group, id, index) {
        ...
        fwThis.addSelect([_keyArea, _keyArea + 1], function (group, id, index) {
            // 当单元格为2到9时(内容为2行时)
            if (group == _keyArea && index > 0 && index < 9) {
                ...
                for (var i = 0; i < textDt.length; i++) {
                    ...
                    new fObject(_keyArea + 1, i, 'hs_keyboard_d' + i,
                        function (obj) {
                            $(fwThis.areaLastFocus[_keyArea].objid).innerHTML = getKeyHtml(getText(fwThis.areaLastFocus[_keyArea].objindex)); // 恢复单元格默认值
                            fwThis.initFocus(fwThis.areaLastFocus[_keyArea].objid, {holdOldFoc: true});
                        },
                        function (obj) {
                            $(fwThis.areaLastFocus[_keyArea].objid).innerHTML = getKeyHtml(getText(fwThis.areaLastFocus[_keyArea].objindex)); // 恢复单元格默认值
                            fwThis.initFocus(fwThis.areaLastFocus[_keyArea].objid, {holdOldFoc: true});
                        ...
                        function (obj) {
                            if (obj.objindex > 0) {
                                fwThis.initFocus('hs_keyboard_d' + (obj.objindex - 1))
                            }
                        },
                        function (obj) {
                ...
                $(id).innerHTML = str;
                fwThis.fObjCommit();
                fwThis.areaLastFocus[_keyArea + 1] = null;
                fwThis.initFocus('hs_keyboard_d0', {holdOldFoc: true});
            } else if (index == 9) {  //  回删
    ...
    this.hiddenKey = function (_newObjid) {
        if (_newObjid) _this.initFocus(_newObjid);
        if (!hs.isDomExist('hs_keyboard')) _this.toggleDisplay($("hs_keyboard"), 'hidden');
    };
    /**
     * 描述: 显示log
    ...
    this.logi = function (msg) {
        ...
        $('hs_log_con').innerHTML += msg + '<br>';

        var height = $('hs_log_con').offsetHeight - $('hs_log_con').parentNode.offsetHeight;
        var nowheight = parseInt(_this.getStyle($('hs_log_con'), "top"));
        if (nowheight > (-height)) {
    ...
    this.base64 = function (jsonData) {
        var b64 = new Base64();
        if (_this.isObject(jsonData)) {
            return b64.encode(fwThis.json.stringify(jsonData));
        } else {
    ...
    this.formPost = function (action, data) {
        ...
        _this.forEach(data, function (item, index) {
    ...
    /**
     * 描述: 大于指定时间返回true, 默认返回false
     * @param beginData  指定时间, 如: 2020-02-28 18:38:01 or  20200228183801
     * @returns {boolean}
     */
    this.getBooleanGreaterThanData = function (beginData) {
        var res = false;
        if (_this.isString(beginData)) {
            beginData = parseInt(beginData.replace(/[\s|\-|:]/gi, ''));
        }
        fwThis.ajax.get('http://21.254.5.190/UnityService/WcfService.svc/GetDateTime?showStyle=40', function (obj) {
            obj = fwThis.json.parse(obj);
            console.log(obj);
            res = parseInt(obj.Data.DateFormat.replace(/[\s|\-|:]/gi, '')) - beginData > 0;
        }, 0, function (msg) {
            console.log(msg);
        }, 3000);
        return res;
    ...
    /**
     * 描述: 小于指定时间返回true, 默认返回false
     * @param beginData  指定时间, 如: 2020-02-28 18:38:01 or  20200228183801
     * @returns {boolean}
     */
    this.getBooleanLessThanData = function (endData) {
        var res = false;
        if (_this.isString(endData)) {
            endData = parseInt(endData.replace(/[\s|\-|:]/gi, ''));
        }
        fwThis.ajax.get('http://21.254.5.190/UnityService/WcfService.svc/GetDateTime?showStyle=40', function (obj) {
            obj = fwThis.json.parse(obj);
            console.log(obj);
            res = endData - parseInt(obj.Data.DateFormat.replace(/[\s|\-|:]/gi, '')) > 0;
        }, 0, function (msg) {
            console.log(msg);
        }, 3000);
        return res;
...
hs.ajax = new AjaxObject();
hs.xml = new XmlObject();
hs.mw = new MediaObject(hs);
hs.cookie = new CookieObject();
hs.localStorage = new LocalStorageObject();
hs.global = new GlobalObject();
hs.json = new JsonObject(hs);
hs.locationUrl = new LocationUrlObject();
hs.memoryO = new MemoryObject(hs);
hs.ListenLive = new ListenLive(hs);
...
/**
 * 描述:  获取dom对象, #:id获取器, .:classname获取器, 其他:有tag就是标签获取器,无默认获取走id获取器
 * @param selector
 * @param root
 * @param tag
 * @returns {(HTMLElement | Array)|null|HTMLCollectionOf<Element>}
 */
window.$ = function (selector, root, tag) {
    root = root || document;
    tag = tag || '*';
    var n, node;
    if (typeof selector == 'string') {
        switch (selector.charAt(0)) {
            case "#":
                ...
                n = document.getElementById(selector);
                break;
            case ".":
                n = [];
                ...
                for (var i = 0; i < nodes.length; i++) {
                    node = nodes[i];
                    if (hs.hasClass(node, selector.substring(1))) {
                        n.push(node);
                    }
                }
                break;
            default:
                n = tag === '*' ? $('#' + selector) : root.getElementsByTagName(tag);
        }
        return n;
    } else {
        return null;
